...
class store_shopslideshow_Adminhtml_shopslideshowController extends Mage_Adminhtml_Controller_Action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('store/shop/shopslideshow/items')
	...
	public function indexAction() {
		$this->_initAction()
			->_addContent($this->getLayout()->createBlock('shopslideshow/adminhtml_shopslideshow'))
			->renderLayout();
	}

	public function editAction() {
		$id     = $this->getRequest()->getParam('id');
		$model  = Mage::getModel('shopslideshow/shopslideshow')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			Mage::register('shopslideshow_data', $model);

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

			$this->_addContent($this->getLayout()->createBlock('shopslideshow/adminhtml_shopslideshow_edit'))
				->_addLeft($this->getLayout()->createBlock('shopslideshow/adminhtml_shopslideshow_edit_tabs'));
		...
		if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != null) {
				...
				try {	
					...
					$path = Mage::getBaseDir('media') . DS.'store/shop/slideshow'.DS ;
		        ...
		        $data['image'] = 'store/shop/slideshow/'.$result['file'];
			}
            ...
            if(isset($_FILES['small_image']['name']) && $_FILES['small_image']['name'] != null) {
		        ...
		        $data['small_image'] = 'store/shop/slideshow/'.$result['file'];
			}
    ...
    public function massDeleteAction() {
        $shopslideshowIds = $this->getRequest()->getParam('shopslideshow');
        if(!is_array($shopslideshowIds)) {
            ...
            try {
                foreach ($shopslideshowIds as $shopslideshowId) {
                    $shopslideshow = Mage::getModel('shopslideshow/shopslideshow')->load($shopslideshowId);
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($shopslideshowIds)) {
            ...
            try {
                foreach ($shopslideshowIds as $shopslideshowId) {
                    $shopslideshow = Mage::getSingleton('shopslideshow/shopslideshow')
