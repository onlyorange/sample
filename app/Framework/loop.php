...
if (have_posts()) :
     ...
     */
    $size = strpos($blog_style, 'big') ? 'entry_with_sidebar' : 'square';
    ...
    echo "<div class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."'>";
            ...
            if(is_singular())
            {
                $link = swedenWpFunctions::image_by_id(get_post_thumbnail_id(), 'large', 'url');
            }
            ...
            $blog_meta_output = "";
            $icon =  '<span class="iconfont"></span>';//.$avia_config['font_icons'][$post_format]."</span>";
            ...
            echo "<div class='entry-content-wrapper clearfix {$post_format}-content'>";
                ...
                echo "<span class='date-container minor-meta updated'>".get_the_time(get_option('date_format'))."</span>";
...
<div class="entry">
    <h2 class='post-title entry-title'><?php _e('Nothing Found', 'sw'); ?></h2>
</div>
...
<?php
    ...
    {
        echo "<div class='{$blog_style}'>".swedenWpFunctions::pagination()."</div>";
    }
?>
