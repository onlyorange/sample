...
// The Site Entity
// contains properties and methods(include helper) used in all templates.
// In templates this entity is available under variable $site.

class swedenWpSiteEntity extends swedenWpBaseEntity
{
    ...
    public function __get($key)
    {
        ...
        try {
            ...
            if (!is_null($result)) {
                ...
                if (property_exists(__CLASS__, $key)) {
                    if (is_null($this->$key)) {
                        $this->$key = get_bloginfo(swedenWpFunctions::camel2underscore($key));
        ...
        } catch (swedenWpEntityException $e) {
            if (property_exists(__CLASS__, $key)) {
                if (is_null($this->$key)) {
                    $this->$key = get_bloginfo(swedenWpFunctions::camel2underscore($key));
    ...
    {
        // hm...should be fine without contructor
    }
    ...
    public static function getInstance()
    {
        ...
        //Handle device detection from Varnish basing on HTTP_X_UA_DEVICE header
        if(isset($_SERVER['HTTP_X_UA_DEVICE']) && $_SERVER['HTTP_X_UA_DEVICE']) {
            self::$instance->isMobile = $_SERVER['HTTP_X_UA_DEVICE']=='mobile' || $_SERVER['HTTP_X_UA_DEVICE']=='tablet'?true:false;
            self::$instance->isTablet = $_SERVER['HTTP_X_UA_DEVICE']=='tablet'?true:false;
            self::$instance->isPhone = $_SERVER['HTTP_X_UA_DEVICE']=='mobile'?true:false;
        // if HTTP_X_UA_DEVICE  header not available use standard detection
        } else {
            $detect = new deviceDetection;
            self::$instance->isMobile = $detect->isMobile();
            self::$instance->isTablet = $detect->isTablet();
            self::$instance->isPhone = ($detect->isMobile() && !$detect->isTablet())?true:false;
        }
    ...
    /**
     * Helper for better manipulation with widgets in templates.
     *
     * Register sidebars for widgets in functions.php like
     * !! prefix is "sidebar-"
     * 'id' => 'sidebar-widgets-1', 'id' => 'sidebar-widgets-2'... something like this.
     *
     ...
     *
     * usage: $site->create('slider', 'homepage', 5)
     *
     * @param string $what
    ...
    /**
     * Creates Entities from Custom Types (static)
     * @param string $what
     * @param string $category
     * @param int $limit
     * @return array|swedenWpPostEntity
    ...
    public static function createCustomPostEntity($what, $category = 0, $limit = null, $order = 'menu_order', $suppress_filters = false)
    {
        ...
        return swedenWp::createPostEntity(new WP_Query(array(
                'tax_query' => $tax,
                'post_type' => 'ait-' . $what,
                'post_status' => 'publish',
                'orderby' => $order,
