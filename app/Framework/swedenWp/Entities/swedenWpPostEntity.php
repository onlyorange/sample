...
// The Post Entity
class swedenWpPostEntity extends swedenWpBaseEntity
{
     ...
     * @var array Array of swedenWpCommentEntity entities
     */
    protected $comments;
     ...
     * @var string
     */
    protected $omniturePageType;
    
    /**
     * Omniture prop2
     * @var string
     */
    protected $omnitureProp2;
    
    /**
     * Omniture prop3
     * @var string
     */
    protected $omnitureProp3;
    ...
    public function __construct($post, $meta = null)
    {
        ...
        if (isset($meta['author']) and $meta['author'] instanceof swedenWpPostAuthorEntity) {
        ...
        $this->metaDescription = $this->post_metaDesc();
        ...
        $pageType = $this->getOmniturePageType();
        $omintureVariables = $this->getOmnitureVariables($pageType);
        
        $this->omniturePageType = $pageType;
        $this->omnitureProp2 = $omintureVariables['prop2'];
        $this->omnitureProp3 = $omintureVariables['prop3'];
    }
    ...
    protected function post_metaDesc()
    {
    	$yoast_meta = get_post_meta($this->id, '_yoast_wpseo_metadesc', true);
    	
    	if (!empty($yoast_meta)) {
    		$cont = $yoast_meta;
    	} else {
    		$cont_post = get_post($this->id);
    		$cont = $cont_post->post_content;
     ...
     * @return int|swedenWpPostAuthorEntity
     */
    protected function getAuthor()
    {
        if (is_int($this->author)) {
            return $this->author = new swedenWpPostAuthorEntity($this->author);
        } else {
            return $this->author;
        }
    }
    ...
    /**
     * Gets comments
     * @return array of swedenWpCommentEntity entities
     */
    protected function getComments()
    {
        ...
        wp_list_comments(array(
            'callback' => 'swedenWpPostEntity::createCommentEntity',
            'walker' => new swedenWpCommentWalker,
        ));
    ...
    public function options($key)
    {
        if (is_array($this->meta) and isset($this->meta[$key])) {
            ...
            if (defined('DEVELOPMENT') && DEVELOPMENT) {
                throw new swedenWpEntityException('The key "' . htmlspecialchars($key, ENT_QUOTES) . '" does not exists.');
            } else {
                return new stdClass;
            }
        }
    ...
    protected function getOptions()
    {
        if (!is_array($this->meta)) {
            ...
            if (defined('DEVELOPMENT') && DEVELOPMENT) {
                throw new swedenWpEntityException('Options are in array. Use method $post->options($key) and $key "' . htmlspecialchars($key, ENT_QUOTES) . '"');
            } else {
                return new stdClass;
            }
        }
    ...
    public static function createCommentEntity($comment, $args, $depth)
    {
        ...
        self::$commentEntities[] = new swedenWpCommentEntity($comment, $args, $depth);
    }
    
    /**
     * Get Omniture PageType based on page template
     * 
     * @return string
     */
    public function getOmniturePageType()
    {
        $defaultPageType = 'DK Article';
        $template = $this->getPageTemplate();
        
        if($this->type == 'sweeps') {
        	$defaultPageType = 'WHS Sweepstakes';
        } else {
        	$pageTypes = array(
        			'DK Home Page' => array(
        					'page-home.php'
        			),
        			'DK Landing Page' => array(
        					'page-category.php'
        			),
        ...
        foreach($pageTypes as $pageType => $templates) {
    ...
    /**
     * Prepare Omniture variables
     * 
     * @return string
     */
    public function getOmnitureVariables($pageType)
    {
        $items = array();
        $variables = array();
        ...
        switch ($pageType) {
            ...
            default:
                $prop2 = $items[0].(isset($items[1])?(" > ".$items[1]):"");
                $prop3 = $items[0].(isset($items[1])?(" > ".$items[1]):"").(isset($items[2])?(" > ".$items[2]):"");
                $variables = array(
                    'prop2' => $prop2,
                    'prop3' => $prop3,
                );
    ...
    {
        $template = (is_single($post) && !is_front_page($post))?'single':get_post_meta( $this->id, '_wp_page_template', true );
