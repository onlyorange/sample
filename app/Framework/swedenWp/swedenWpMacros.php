 ...
 * `{cufon fonts, fancyFont, "$themeUrl/static/js/libs/cufon.js", THEME_FONTS_URL . "/{$themeOptions->fonts->fancyFont->file}", $themeOptions->fonts->fancyFont->font, $themeOptions->general->displayThemebox}`
 ...
 *
 */
class swedenWpMacros extends NMacroSet {
    /**
     * Install all macros
     *
    ...
    public static function install(NParser $parser) {
        ...
        $me->addMacro('title', 'echo swedenWpFunctions::getTitle()');

        $me->addMacro('editPostLink', 'edit_post_link(__("Edit", "ait"), "<span class=\"edit-link\">", "</span>", %node.word);');

        $me->addMacro('editCommentLink', 'swedenWpFunctions::editCommentLink(__("Edit", "ait"), %node.word, "<span class=\"edit-link\">", "</span>");');
        ...
        // oh my, if there was $node->tokenizer->fetchArray(), so life would be better
        ...
        //AK: though these don't do anything, leaving them in because they seem to be linked to Nette ... ?
        $me->addMacro('less', 'echo swedenWpFunctions::lessify()');
        $me->addMacro('generatedCss', 'echo swedenWpFunctions::generatedCss()');

        $me->addMacro('breadcrumbs', 'echo swedenWpFunctions::breadcrumbs(%node.array)');

        $me->addMacro('lang', 'echo do_shortcode(%node.args)');

        $me->addMacro('dayLink', 'echo swedenWpFunctions::getDayLink(%node.args)');
        ...
        $me->addMacro('paginateLinks', 'echo swedenWpFunctions::paginateLinks(%node.args)');

        $me->addMacro('stripTag', 'echo swedenWpFunctions::stripTag(%node.args);');
        ...
        // thumbnail resizing...disabled
        $me->addMacro('thumbnailResize', '');

        $me->addMacro('widget', 'the_widget(%node.word, %node.array)');

        $me->addMacro('includePart', array($me, 'macroIncludePart'));
        $me->addMacro('getSingleNav', 'echo swedenWpFunctions::getSingleNav()');
        $me->addMacro('truncate', 'echo swedenWpFunctions::backend_truncate(%node.args)');
        
        $me->addMacro('getOptionValue', 'echo swedenWpFunctions::getOptionValue(%node.args)');
    }

    /**
     * Prepare to parse things from template
     *
     * @param NMacroNode $node
    ...
    public static function getTemplatePart($slug, $name = null) {
        ...
        if($name)
            $templates[] = "templates/{$slug}-{$name}.html"; // hardcoded Templates dir :( // but it's more wordpress-like :S
