<?php
/**
* Base shortcode templating functions
*
* element type and popup editor are copied from enfold
*/

// Don't load directly
if ( !defined('ABSPATH') ) { die('-1'); }

if ( !class_exists( 'swedenShortcodeTemplate' ) ) {

    abstract class swedenShortcodeTemplate
    {
        var $builder;
        var $config  = array();
        ...
        //init function is executed in swedenBuilder::createShortcode if the shortcode is allowed
        ...
        *	$this->config['name'] = __('Text', 'swedenWp' ); //defines the name of the button that is displayed bellow the icon
        *   $this->config['tab'] = __('Layout Elements', 'swedenWp' ); //tab that should hold the button
        ...
        *   $this->config['tooltip'] = __('This is a tooltip', 'swedenWp' ); //the tooltip that appears when hovering above the shortcode icon
        ...
        */
        public function popup_editor($var)
        {
            ...
            echo swedenHtmlHelper::render_multiple_elements($elements, $this);
        ...
        */
        public function shortcode_handler_prepare($atts, $content = "", $shortcodename = "", $fake = false)
        {
            ...
            if(empty($this->config['inline'])) //inline shortcodes like dropcaps are basically nested shortcodes and should therefore not be counted
            {
                ...
                $fullwidth = array('av_section','av_layerslider','av_slideshow_full', 'av_fullscreen');
        ...
        protected function extra_config_element_iterator($elements)
        {
            ...
            foreach($elements as $element)
            {
                switch($element['type'])
                {
                    ...
                    case "title_tiny_mce": 	$this->config['modal_on_load'][] = 'modal_load_tiny_mce_title'; break;
        ...
        protected function create_asset_array()
        {
            if(!empty($this->elements))
            {
                foreach ($this->elements as $element)
                {
                    ...
                    {
                        swedenBuilder::$resources_to_load[$element['font']] = $element;
                    }
                }
            }
        }
        ...
        * which in turn returns the array with the properties to render a new swedenBuilder Canvas Element
        */
        ...
        */
        ...
        {
            $elements[] = array(
                "name" 	=> __("Custom Css Class",'swedenWp' ),
                "desc" 	=> __("Add a custom css class for the element here. Make sure to only use allowed characters (latin characters, underscores, dashes and numbers)",'swedenWp' ),
                "id" 	=> "custom_class",
                "type" 	=> "input",
                "std" 	=> "");
        ...
        */
        public function create_sortable_editor_element($params)
        {
            ...
            if(!empty($this->config['popup_editor']))
            {
                ...
                $output .= "<a class='$extraClass'  href='#edit-element' title='".__('Edit Element','swedenWp' )."'>edit</a>";
            }

            //$output .= "<a class='avia-new-target'  href='#new-target' title='".__('Move Element','swedenWp' )."'>+</a>";
            $output .= "<a class='avia-delete'  href='#delete' title='".__('Delete Element','swedenWp' )."'>x</a>";
            $output .= "<a class='avia-clone'  href='#clone' title='".__('Clone Element','swedenWp' )."' >".__('Clone Element','swedenWp' )."</a></div>";
        ...
        /**
         * helper function executed by swedenShortcodeTemplate::popup_editor that extracts the attributes from the shortcode and then merges the values into the options array
         *
         ...
         * and data attributes for an swedenBuilder Canvas element in your backend
         *
