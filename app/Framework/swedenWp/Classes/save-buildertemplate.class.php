<?php
/**
 * Logic for creating and saving template
 *
 * Using same code / logic from enfold
 */

// Don't load directly
if ( !defined('ABSPATH') ) { die('-1'); }

if ( !class_exists( 'swedenLayoutSaver' ) ) {

    class swedenLayoutSaver
    {
       var $builder;
        ...
        */
        protected function actions_and_filters()
        {
            ...
            #js
            wp_enqueue_script('avia_template_save_js' , swedenBuilder::$path['assetsURL'].'js/avia-template-saving.js' , array('avia_element_js'), $ver, TRUE );
        ...
        */
        public function create_save_button()
        {
            ...
            if(empty($names))
            {
                $list = "<li class='avia-no-template'>" .__('No Templates saved yet','swedenWp' ) ."</li>\n";
            }
            else
            {
                foreach($names as $name)
                {
                    $list .= "<li><a class='{$name}' href='#'>{$name}</a><span class='avia-delete-template'></span></li>\n";
                }
            }

            $output  = "";
            $output .= "<div class='avia-template-save-button-container avia-attach-template-save avia-hidden-dropdown'>";
            $output .= "    <a class='open-template-button button' href='#open'>".__('Templates','swedenWp' )."</a>";
            $output .= "    <div class='avia-template-save-button-inner'> <span class='avia-arrow'></span>";
            $output .= "        <a class='save-template-button button button-primary button-large' href='#save'>".__('Save Entry as Template','swedenWp' )."</a>";
            $output .= "        <div class='avia-template-list-wrap'>";
            $output .= "            <span class='avia-tempaltes-miniheader'>".__('Load Template','swedenWp' ).":</span>";
            $output .= "            <ul>";
            $output .=                 $list;
            $output .= "            </ul>";
            $output .= "        </div>";
        ...
        public function save_builder_template($name = "%", $value = "")
        {
            ...
            $id     = swedenStoragePost::get_custom_post('template_builder_snippets');
            ...
            if(!empty($old))
            {
                echo __('Template name already in use. Please delete the template with this name first or choose a different name', 'swedenWp' );
            }
            else
            {
