<?php
/**
 * metabox functions for enfold
 *
 * this possibly conflicts with sweden framework.
 */
...
if ( !class_exists( 'MetaBoxBuilder' ) ) {
    ...
    class MetaBoxBuilder
    {
        ...
        function add_js_info()
        {
            ...
            echo "    avia_globals.builderversion = '".swedenBuilder::VERSION."';\n";
            echo "    avia_globals.builderMode = '".swedenBuilder::$mode."';\n";
            echo "/* ]]> */ \n";
            echo "</script>\n \n ";
        }
            ...
            if(!empty($this->default_boxes) && !empty($this->box_elements))
            {
                ...
                foreach($this->default_boxes as $key => $box)
                {
                    foreach ($box['page'] as $area)
                    {
                        ...
                        if(('avia_builder' === $box['id'] && isset($_GET['post']) && get_post_meta($_GET['post'], '_aviaLayoutBuilder_active', true) != 'active') || ('avia_builder' === $box['id'] && empty($_GET['post'])))
                        {
                        ...
                        add_meta_box(
                            ...
                            'high', // $box['priority'],						// The priority within the context where the boxes should show ('high' or 'low')
        ...
        function create_meta_box($currentPost, $metabox)
        {
            ...
            if(!empty($box['expandable']))
            {
                $title = __('Expand','swedenWp' )." ".$box['title'];
                $close = __('Close','swedenWp' );
            ...
            foreach ($this->box_elements as $element)
            {
                ...
                if($element['slug'] == $box['id'])
                {
                    ...
                    else if (method_exists('swedenHtmlHelper', $element['type']))
                    {
                        $content = swedenHtmlHelper::render_metabox($element);
                    }
                }
        ...
        function save_post($id, $post_object)
        {
            ...
            // saving revision for any updates
            /*
