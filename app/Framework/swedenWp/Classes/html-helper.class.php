...
if ( !class_exists( 'swedenHtmlHelper' ) ) {

    class swedenHtmlHelper
    {
        ...
        static function render_element($element, $parent_class = false)
        {
            ...
            if(!empty($element['target']))
            {
                ...
                $target_string = swedenWpFunctions::create_data_string($data);
        ...
        */
        static function check_dependencies($element)
        {
            ...
            if(!empty($element['required']))
            {
                ...
                $params['data_string'] 		= swedenWpFunctions::create_data_string($data);
                $return = false;
        ...
        static function modal_group($element, $parent_class)
        {
            ...
            $output .= "</div>";
            $output .= "<a class='avia-attach-modal-element-add avia-add'>".__('Add','swedenWp' )."</a>";
        ...
        static function modal_group_sub($element, $parent_class, $i = false)
        {
            ...
            $dataString  	= swedenWpFunctions::create_data_string($data);


            $output .= "<div class='avia-modal-group-element ' {$dataString}>";
            $output .= "<a class='avia-attach-modal-element-move avia-move-handle'>".__('Move','swedenWp' )."</a>";
            $output .= "<a class='avia-attach-modal-element-delete avia-delete'>".__('Delete','swedenWp' )."</a>";
        ...
        static function tiny_mce($element)
        {
            ...
            wp_editor( $element['std'], $element['id'] , array(
                                        'editor_class' => 'avia_advanced_textarea avia_tinymce',
         ...
         * @return string $output the string returned contains the html code generated within the method
         */
        static function title_tiny_mce($element) {
            //tinymce only allows ids in the range of [a-z] so we need to filter them.
            $element['id']  = preg_replace('![^a-zA-Z_]!', "", $element['id']);
            //var_dump($element['std']);
            /* monitor this: seems only ajax elements need the replacement */
            $user_ID = get_current_user_id();
            ...
            if (get_user_meta($user_ID, 'rich_editing', true) == "true" && isset($element['ajax'])) {
                //replace new lines with brs, otherwise the editor will mess up
                $element['std'] = str_replace("\n",'<br>',$element['std']);
                $element['std'] = htmlspecialchars_decode($element['std']);
            }
        ...
        static function wrapper_close() {
            $out = '</div>';

            return $out;
        }
        static function nextBtn() {
            $out = '<div id="validationMessage"></div><div class="nextBtn button" id="productValidation"><span>Next</span></div>';

            return $out;
        }
        static function carousel_nextBtn() {
        	$out = '<div id="validationMessage"></div><div class="nextBtn button" id="productValidationCarousel"><span>Load Product Details</span></div>';
        ...
        static function titlepicker($element) {
            $output = '<input type="text" class="textTitle '.$element['class'].'" value="'.$element['std'].'" id="'.$element['id'].'" name="'.$element['id'].'"/>';
        ...
        static function linkpicker($element)
        {
            ...
            $product_std = explode(',', $element['std']);

            $pt = array_flip(swedenWpFunctions::public_post_types());
            $ta = array_flip(swedenWpFunctions::public_taxonomies(false, true));
            ...
            if(in_array('product', $element['subtype']))
            {
            	//echo 'blah';
            }
            if(in_array('product_sync', $element['subtype']))
            {
            	//echo 'blah';
            }
            if(in_array('product_carousel', $element['subtype']))
            {
            	//echo 'blah';
            }
            if(in_array('single', $element['subtype']))
            {
                foreach($pt as $key => $type)
                {
                    if(in_array($type, $allowed_pts))
                    {
                        ...
                        if ($html) { swedenWpFunctions::register_template($type, $html); } else { unset($pt[$key] ); }
                    }
                    else
                    {
                        unset($pt[$key]);
            ...
            if(in_array('taxonomy', $element['subtype']))
            {
                ...
                foreach($ta as $key => $type)
                {
                    if(in_array($type, $allowed_tas))
                    {
                        ...
                        $html = self::select($original);
                        if ($html) {swedenWpFunctions::register_template($type, $html); } else { unset($ta[$key] ); }
                    }
                    else
                    {
                        unset($ta[$key]);
            ...
            foreach($element['subtype'] as $value => $key) //register templates
            {
                ...
                {
                    case "manual":
                        if($new_std[0] != $key) $element['std'] = "";
                        $original['subtype'][$value] = $key;
                        $html = self::input($element);
                        swedenWpFunctions::register_template($key, $html);
                    break;
                    ...
                    case "product_carousel":
                    	if($product_std[0] != $key) $element['std'] = "";
                    	$original['subtype'][$value] = $key;

                    	$html = '<label>Product ID</label><input type="text" class="ATGInfo" value="'.$product_std[1].'" id="aviaTBlink" name="aviaTBlink">
                        		<label>Product SKU</label><input type="text" class="ATGInfo" value="'.$product_std[2].'" id="aviaTBlink" name="aviaTBlink">
                        		<label>Product Country</label><input type="text" class="ATGInfo" value="'.$product_std[3].'" id="aviaTBlink" name="aviaTBlink">
                        		<label>Product Language</label><input type="text" class="ATGInfo" value="'.$product_std[4].'" id="aviaTBlink" name="aviaTBlink">
                        		<div id="GetProductInfo" class="button" style="margin-top:10px;"><span>Get Product Information</span></div>
            ...
            {
                swedenWpFunctions::print_templates();
            }
        ...
        static function linkpickerB($element)
        {
        	$original = $element;
            $new_std = explode(',', $element['std'], 4);
            $product_std = explode(',', $element['std']);
        ...
        static function image($element)
        {
            if(empty($element['data']))
            {
                ...
                {
                    $class = $fetch == "id" ? "avia-media-img-only" : "avia-media-img-only";
                }
            ...
            $data 	= swedenWpFunctions::create_data_string($element['data']);
            $class 	= 'button swedenBuilder-image-upload avia-builder-image-insert '.$element['class'];
            ...
            if(isset($fetch))
            {
            	if(strpos($element['id'], 'video')) {
            		$attachmentType = 'attachment_video';
            	} else {
            		$attachmentType = 'attachment';
            	}
                $fake_img_id = str_replace ( str_replace('aviaTB','',$element['id']) ,'img_fakeArg', $element['id']);
                $img_id_field = str_replace ( str_replace('aviaTB','',$element['id']) , $attachmentType, $element['id']);

                $fake_img = $fetch == "id" ? wp_get_attachment_image( $element['std'], 'thumbnail') : '<img src="'.$element['std'].'" />';
                $attchedID = swedenWpFunctions::get_attachment_id_from_url($element['std']);
                ...
                $output .= '<input type="hidden" class="hidden-'.$attachmentType.'-id '.$element['class'].'" value="'. $attchedID .'" id="'.$img_id_field.'" name="'.$img_id_field.'"/>';
            }

            return $output;
        }

        // mobile fall-back image uploader
        static function image_mobile($element) {
        	if(empty($element['data'])) {
        		$fetch = isset($element['fetch']) ? $element['fetch'] : "url";
        		$state = isset($element['state']) ? $element['state'] : "avia_insert_single";
        	...
        	//fake img for multi_image element
        	if(isset($fetch)) {
				$attachmentType = 'mobile-attachment';
        		$fake_img_id = str_replace ( str_replace('aviaTB','',$element['id']) ,'img_fakeArg', $element['id']);
        		$img_id_field = str_replace ( str_replace('aviaTB','',$element['id']) , $attachmentType, $element['id']);
        ...
        static function image_title($element)
        {
            if(empty($element['data']))
            {
                $fetch = isset($element['fetch']) ? $element['fetch'] : "url";
            ...
            $data 	= swedenWpFunctions::create_data_string($element['data']);
            $class 	= 'button swedenBuilder-image-title-upload avia-builder-image-insert '.$element['class'];
            $output = '	<a href="#" class="'.$class.'" '.$data.' title="'.esc_attr($element['title']).'">'.$element['title'].'</a>';
            ...
            {
                $fake_img_id = str_replace ( str_replace('aviaTB','',$element['id']) ,'img_fakeArg', $element['id']);
                $img_id_field = str_replace ( str_replace('aviaTB','',$element['id']) ,'attachment_title', $element['id']);

                $fake_img = $fetch == "id" ? wp_get_attachment_image( $element['std'], 'thumbnail') : '<img src="'.$element['std'].'" />';
                $attchedID = swedenWpFunctions::get_attachment_id_from_url($element['std']);

                $output .= '<input type="hidden" class="hidden-image-url '.$element['class'].'" value="'.htmlentities($fake_img, ENT_QUOTES, get_bloginfo( 'charset' )).'" id="'.$fake_img_id.'" name="'.$fake_img_id.'"/>';
                $output .= '<input type="hidden" class="hidden-title-attachment-id '.$element['class'].'" value="'.$attchedID.'" id="'.$img_id_field.'" name="'.$img_id_field.'"/>';
            }

            return $output;
        }
        // mobile fall-back title image uploader
        static function image_title_mobile($element) {
        	if(empty($element['data'])) {
        		$fetch = isset($element['fetch']) ? $element['fetch'] : "url";
        		$state = isset($element['state']) ? $element['state'] : "avia_insert_single";
        ...
        static function gallery($element)
        {
            ...
            return swedenHtmlHelper::image($element);
        }
        ...
        static function select($element)
        {
            ...
            else if($element['subtype'] == 'jetSet')
            {
                $args = array(
                        'posts_per_page'   => 9999,
                        'offset'           => 0,
            ...
            else if($element['subtype'] == 'fashionStyle')
            {
                $args = array(
                        'posts_per_page'   => 9999,
                        'offset'           => 0,
        ...
        static function table($element , $parent)
        {
            ...
            $output .= "		<a class='avia-attach-table-row button button-primary button-large'>".__('Add Table Row','swedenWp' )."</a>";
            $output .= "		<a class='avia-attach-table-col button button-primary button-large'>".__('Add Table Column','swedenWp' )."</a>";
            $output .= "	</div>";
        ...
        static function display_image($img = "")
        {
            ...
            $output .= "</div>";
            $output .= "<a href='#delete' class='avia-delete-image {$hidden}'>".__('Remove Image', 'swedenWp' )."</a>";
