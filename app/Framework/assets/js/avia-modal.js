...
{
	"use strict";
   var stage = (/stage|dev/).test(mkDomain);
    ...
    $.AviaModal.openInstance = [];
   	...
   	$.AviaModal.prototype =
   	{
   		...
   		add_behavior: function()
   		{
   			...
   			// validate user input
   			// General CTA product info sync
   			this.modal.on('click', '#GetProductInfo', function(){

               var postData = {
                  productId: $('#aviaTBlink.ATGInfo:eq(0)').val(),
   				   skuId: $('#aviaTBlink.ATGInfo:eq(1)').val(),
   				   country: $('#aviaTBlink.ATGInfo:eq(2)').val(),
   				   lang: $('#aviaTBlink.ATGInfo:eq(3)').val(),
                  route: (stage) ? '/api/v1/product' : undefined
               };

               var url = (stage) ? '/wp-content/themes/destinationkors/proxy.php' : 'http://' + mkDomain + '/api/v1/product';

   				$('#validationMessage').text('Validating product info. Please wait...');
                ...
                $.ajax({
                  ...
                  xhrFields: {
   					...
   					success: function(data) {
   						if(data['success']) {
   							console.log(data);
   							$('#validationMessage').html('<img src="' + data['product']['imageURL'] + '">'
   									+ '<Br/>Short Descrition: ' + data['product']['shortDescription']
   									+ '<Br/>List Price: ' + data['product']['listPrice']
   									+ '<Br/>Sale Price: ' + data['product']['salePrice']
   									+ '<Br/>Currency Code: ' + data['product']['currencyCode']);
   						} else {
   							$('#validationMessage').text('There is no matching product.');
   						}
   					},
   					error: function (xhr, status, err) {
   						$('#validationMessage').text('Can not validate product infomation.');
   						console.log(status, err);
   					}
   				});
   			});

   			this.modal.on('click', '#productValidationCarousel', function(){
   				var postData = {
                  productId: $('#aviaTBaviaTBp_id').val(),
   				   skuId: $('#aviaTBaviaTBsku').val(),
   			      country: $('#aviaTBaviaTBcountry').val(),
   				   lang: $('#aviaTBaviaTBlang').val(),
                  route: (stage) ? '/api/v1/product' : undefined
               };
               ...
               $.ajax({
                  ...
                  xhrFields: {
   					...
   					success: function(data) {
   						if(data['success']) {
   							$('#aviaTBaviaTBcontent_wrapper_first-form-container').hide();
   							$('#aviaTBaviaTBcontent_wrapper_second-form-container').show();

   							if($('#wp-aviaTBaviaTBcontent-editor-container .wp-editor-area').text() == '') {
   								tinyMCE.execCommand('mceInsertContent', true, data['product']['shortDescription']);
   							} else {
   								//
   							}
   							$('#aviaTBaviaTBcontent_wrapper_second-form-container .avia-builder-prev-img-container').html('<span class="avia-builder-prev-img"><img src="' + data['product']['imageURL'] + '"></span>');
   							$('#aviaTBaviaTBcontent_wrapper_second-form-container #aviaTBaviaTBsrc').val(data['product']['imageURL']);
   							$('#aviaTBaviaTBcontent_wrapper_second-form-container #aviaTBaviaTBlist_price').val(data['product']['listPrice']);
   							$('#aviaTBaviaTBcontent_wrapper_second-form-container #aviaTBaviaTBsale_price').val(data['product']['salePrice']);
   							//$('#aviaTBcontent_wrapper_second-form-container #aviaTBlink').val(data['product']['imageURL']);
   						} else {
   							$('#validationMessage').text('There is no matching product.');
   						}
   					},
   			...
   			this.modal.on('click', '#productValidation', function(){

   				var postData = {
                  productId: $('#aviaTBp_id').val(),
                  skuId: $('#aviaTBsku').val(),
                  country: $('#aviaTBcountry').val(),
                  lang: $('#aviaTBlang').val(),
                  route: (stage) ? '/api/v1/product' : undefined
               };
               ...
               $.ajax({
                  ...
                  xhrFields: {
   					...
   					success: function(data) {
   						if(data['success']) {
   							$('#aviaTBcontent_wrapper_first-form-container').hide();
   							$('#aviaTBcontent_wrapper_second-form-container').show();

   							if($('#wp-aviaTBcontent-editor-container .wp-editor-area').text() == '') {
   								tinyMCE.execCommand('mceInsertContent', true, data['product']['shortDescription']);
   							} else {
   								//
   							}

   							$('#aviaTBcontent_wrapper_second-form-container .avia-builder-prev-img-container').html('<span class="avia-builder-prev-img"><img src="' + data['product']['imageURL'] + '"></span>');
   							$('#aviaTBcontent_wrapper_second-form-container #aviaTBimgsrc').val(data['product']['imageURL']);
   							$('#aviaTBcontent_wrapper_second-form-container #aviaTBlist_price').val(data['product']['listPrice']);
   							$('#aviaTBcontent_wrapper_second-form-container #aviaTBsale_price').val(data['product']['salePrice']);
   							//$('#aviaTBcontent_wrapper_second-form-container #aviaTBlink').val(data['product']['imageURL']);
   						} else {
   							$('#validationMessage').text('There is no matching product.');
   						}
			...
			{
   				$('select#aviaTBlinka').addClass('this');
		...
		textareas.each(function()
		{
			...
			switch_btn.bind('click', function()
			{
				...
				else
				{
					...
					if(tinyVersion >= 4) textarea.val( window.switchEditors._wp_Nop( textarea.val() ) );
				}
			}).trigger('click');
			...
			if(tinyVersion >= 4)
			{
				$doc.bind('avia_modal_close.tinymce_remove', function()
				{
					window.tinyMCE.execCommand(executeRem, true, el_id);
					$doc.unbind('avia_modal_close.tinymce_remove');
				});
			}
	...
	/*
	$.AviaModal.register_callback.modal_load_tiny_mce_title = function(textareas) {
		textareas = textareas || this.modal.find('.avia-modal-inner-content .avia_tinymce');
		var modal    = textareas.parents('.avia-modal:eq(0)'),
			save_btn = modal.find('.avia-modal-save'),
			$doc	 = $(document);
