...
(function($)
{
	...
	$.AviaElementBehavior.wp_media_advanced =  function()
	{
		...
		$body.on('click', '.swedenBuilder-image-upload', function( event )
		{	
			console.log('trigger regular');
			event.preventDefault();
						...
						if(options.fetch == 'url')
						{
							...
							else // image insert
							{
								...
								if(attachmentIdwhenSRC.length)
								{
									...
									console.log(attachmentIdwhenSRC.val(element.id));
								} else {
									attachmentIdwhenSRC.val(element.id);
								}
							}
		...
		// delete functionality
		$body.on('click', '.avia-form-element-container .avia-delete-image', function( event ) {
			event.preventDefault();
		...
		$body.on('click', '.swedenBuilder-image-title-upload', function( event )
				{	
					...
					var clicked = $(this), 
						...
						parent 	= clicked.parents('.avia-form-element-container:last'),
		 				target 	= parent.find('#'+options.target),
		 				fakeImg = target.next('input'),
		 				attachmentIdwhenSRC = parent.find('.hidden-title-attachment-id'),
		 				preview = parent.find('.avia-builder-prev-img-container'),
		 				template = parent.find(".avia-tmpl-modal-element").html(),
		 				modal_group = parent.find('.avia-modal-group'),
		 				del_btn	= parent.find('.avia-delete-image'),
