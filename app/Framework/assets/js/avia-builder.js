...
(function($) {
    ...
    $.swedenBuilder = function() {
        ...
        this.shortcodes = $.swedenBuilder.shortcodes || {};
        ...
        //button to switch between wordpress editor and avia builder
        this.switch_layout = $('#layoutType, .avia-tab-1 a');
    ...
    $.swedenBuilder.prototype = {
        ...
        add_behavior: function() {
            ...
            /*
			this.switch_layout.on('click', function(e) {
				e.preventDefault();
				if($('.avia-controll-bar #layoutType').hasClass('waterfall')) {
					$('.avia-controll-bar #layoutType').removeClass('waterfall').addClass('paginated').html('Paginated');

				} else {
					$('.avia-controll-bar #layoutType').removeClass('paginated').addClass('waterfall').html('Waterfall');
				}
			});
			*/

            //add a new element to the swedenBuilder canvas
            this.shortcode_wrap.on('click', '.shortcode_insert_button', function() {
                ...
                /*
				parent.find('.avia-active-insert').removeClass('avia-active-insert');
				...
				{
					this.className += " avia-active-insert"
				}
				else
				{
					obj.targetInsertInactive();
            ...
            //target mode functions: currently disabled. somehow they appear less usefull with the new drag and drop system

            //move item
            /*
			this.canvas.on('click', 'a.avia-new-target', function()
						{
							obj.targetCutItem(this, obj);
							return false;
						});


			//target insert of an element
			this.canvasParent.on('click', '.av_drop', function()
			{
				if(obj.targetInsert)
				{
					obj.doTargetInsert(this, obj);
					return false;
				}
			});

			//disable target mode if right mouse button is clicked
			...
			//keboard buttons for target inserting
			$(document).keydown(function(e)
			{
				if(obj.targetInsert)
				{
					//escape button pressed when in target insert mode: deactivate target insert mode
			  		if (e.keyCode == 27) { setTimeout( function(){ obj.targetInsertInactive(); }, 150);  }

			  		//move target insert element to prev next item. 37 = left, 39 = right
			  		if (e.keyCode == 37 || e.keyCode == 39 )
			  		{
			  			e.preventDefault();
			  			setTimeout( function(){ obj.changeTargetingShortcode(e.keyCode); }, 150);
			  		}
			  	}
			});
			*/
            ...
            var obj = this,
                ...
                params = {
                    ...
                    drop: function(event, ui) {
                        ...
                        var elements = dropable.find('>.av_drag'),
                            offset = {}, method = 'after',
                            toEl = false,
                            position_array = [],
                            last_pos, max_height;
                        ...
                        if ($('.avia_sortable_element.sw_paginated') && $(ui.draggable).hasClass('sw-waterfall')) {
                            var item = $('.avia_sortable_element.sw_paginated, .avia_sortable_element.sw_waterfall');
                            ...
                            item.hide(200, function() {
                                ...
                                if (parent && parent.length) {
                                    console.log("Update inner html paginated");
                                    obj.updateInnerTextarea(parent);
                                }
                                obj.updateTextarea();

                                //bugfix for column delete that renders the canvas undropbable for unknown reason
                                if (obj.secureContent.val() == "") {
                                    obj.activate_element_dropping(obj.canvasParent, "destroy");
                                }

                                obj.do_history_snapshot();
                            });
                        }
                        if ($('.avia_sortable_element.sw_waterfall') && $(ui.draggable).hasClass('sw-paginated')) {
                            var item = $('.avia_sortable_element.sw_paginated, .avia_sortable_element.sw_waterfall');
                            ...
                            item.hide(200, function() {
                                item.remove();
                                if (parent && parent.length) {
                                    console.log("Update inner html waterfall");
                                    obj.updateInnerTextarea(parent);
                                }
                                obj.updateTextarea();
                        ...
                        if ($('.avia_scope.avia_meta_box.avia_meta_box_visual_editor').hasClass('waterfall')) {
                            $('.avia_scope.avia_meta_box.avia_meta_box_visual_editor').addClass('paginated').removeClass('waterfall');
                            $('#layoutType').html('Layout Type Updated!');
                        } else {
                            $('.avia_scope.avia_meta_box.avia_meta_box_visual_editor').addClass('waterfall').removeClass('paginated');
                            $('#layoutType').html('Layout Type Updated!');
                        }
                        /*
						if((obj.canvas).hasClass('sw_waterfall')) {
							$(obj.canvas).addClass('sw_paginated').removeClass('sw_waterfall');

							$('.shortcode_button_wrap.avia-tab-container').addClass('paginated').removeClass('waterfall');
							$('#layoutType').html('Layout Type Updated!');
						}
						*/
                        ...
                        if (insertedInto.data('dragdrop-level') !== 0) {
                            ...
                            //console.log(ui.draggable.data('dragdrop-level'));
                            //console.log(toEl.parents('.av_drop').data('dragdrop-level'));
                            //if(!(ui.draggable.data('dragdrop-level')==3&&toEl.parents('.av_drop').data('dragdrop-level')==1)){
                            ...
                            //}
                            avia_log("update inner 2");

                            obj.updateInnerTextarea(ui.draggable);
                            //if(!(ui.draggable.data('dragdrop-level')==3&&toEl.parents('.av_drop').data('dragdrop-level')==1)){
                            var row = ui.draggable.parents('.avia_layout_row:eq(0)');
                            // var full = ui.draggable.parents('.av_full_bleed:eq(0)');
                            if (row && row.length) {
                                // avia_log("update inner text of row");
                                // avia_log(row);
                                avia_log("update inner 4");

                                obj.updateInnerTextarea(false, $(row[0]));
                            }
                            // if (full && full.length) {
                            // 	console.log("update inner full");
                            // 	console.log(full);
                            // 	console.log(row);
                            // 	obj.updateInnerTextarea(false, $(full[0]));
                            // }
                        }
                        if ((ui.draggable.data('dragdrop-level') == 3 && toEl.parents('.av_drop').data('dragdrop-level') == 1)) {
                            obj.updateTextarea();
        ...
        /**
         * Switches between the wordpress editor and the swedenBuilder editor
         *
         */
        switch_layout_mode: function() {
            ...
            if (this.activeStatus.val() != 'active') {
                ...
                $('#freeFormContent').removeClass('hide-editor');
                ...
                $('#freeFormContent').addClass('hide-editor');
        ...
        /**
         * Send element(s) to the swedenBuilder Canvas
        ...
        updateInnerTextarea: function(element, container) {
            avia_log("update inner text area ");
            ...
            var layout_size = "one_full";
            
            // if (container.is('.av_full_bleed')) {
            // 	layout_size = "full_bleed";
            // }
            if (container.is('.av_one_full')) {
                console.log("one full");
                layout_size = "one_full";
            } else if (container.is('.av_one_half')) {
                console.log("one half");
                layout_size = "one_half";
            } else if (container.is('.av_one_third')) {
                console.log("one third");
                layout_size = "one_third";
            } else if (container.is('.av_two_third')) {
                console.log("two third");
                layout_size = "two_third";
            } else if (container.is('.av_one_fourth')) {
                console.log("one fourth");
                layout_size = "one_fourth";
            }
            ...
            if (container.is('.avia_layout_column') && !container.is('.avia_layout_row')) {
                var main_storage = container.find('>.avia_inner_shortcode >' + this.datastorage),
                ...
                for (var i = 0; i < content_fields.length; i++) {
                    var inner_content = $(content_fields[i]).val();
                    inner_content = inner_content.replace("]", " layout_size='" + layout_size + "']");
                ...
                var row = container.parents('.avia_layout_row:eq(0)');
                if (row && row.length) {
                    //avia_log("update inner text");
                    //avia_log(row);
                    //avia_log("update inner 4");

                    //this.updateInnerTextarea(false, $(row[0]));
                }

            }


            if (container.is('.avia_layout_row')) {
                var columns = container.find('.avia_layout_column');
                for (var i = 0; i < columns.length; i++) {
                ...
                var main_storage = container.find('>.avia_inner_shortcode >' + this.datastorage),
                    content_fields = container.find('>.avia_inner_shortcode > div ' + this.datastorage + ':not(.avia_sortable_element ' + this.datastorage + ')'),
                    content = "",
                    currentSize = container.data('width'),
                    currentFirst = container.is('.avia-first-col') ? " first" : "";
                ...
                console.log("conent field le");
                console.log(content_fields.length);
                for (var i = 0; i < content_fields.length; i++) {
                    content += $(content_fields[i]).val();
                }
         ...
         * the swedenBuilder canvas. only executed at page load or when the editor is switched from default wordpress to avia builder
         */
        convertTextToInterface: function(text) {
            ...
            if (typeof text == "undefined") {
                text = this.secureContent.val();
                if (text.indexOf('[') === -1) {
                    text = this.classic_textarea.val();
        ...
        load_template_preset: function(source) {
            var obj = this;
            $.ajax({
                type: "POST",
                url: ajaxurl,
                ...
                error: function() {
                    new $.AviaModalNotification({
                        mode: 'error',
                        msg: avia_modal_L10n.ajax_error
                    });
                },
                success: function(response) {
                    if (response == 0) {
                        new $.AviaModalNotification({
                            mode: 'error',
                            msg: avia_modal_L10n.login_error
                        });
                    } else if (response == "-1") {
                        new $.AviaModalNotification({
                            mode: 'error',
                            msg: avia_modal_L10n.timeout
                        });
                    } else {
                        obj.set_template(response);
                        switch (source) {
                            // concept
                            case 'Mks Edit':
                                $('#aviaLayoutBuilder div .menu-item-handle a.avia-delete').hide();
                                $('#aviaLayoutBuilder div').removeClass('ui-draggable');
                                break;
                            case 'Jet Set':
                                $('#aviaLayoutBuilder div .menu-item-handle a.avia-delete').hide();
                                //$('#aviaLayoutBuilder div').removeClass('av_drag').removeClass('ui-draggable').removeClass('avia_sortable_element');
                                break;
        ...
        check_up_page: function() {
            var obj = this,
                content = $('#edit-slug-box #sample-permalink').text().length,
                content_double_check = $('.layout-builder-wrap #aviaLayoutBuilder').has('div').length,
                mksEdit = 'post-type-mks-edit',
                jetSet = 'post-type-jet',
                fashion = 'post-type-fashion',
                templateMksEdit = 'Mks Edit',
                templateJetSet = 'Jet Set',
                templateFashion = 'Fashion and Style';

            console.log('content_double_check');
            if ($('.avia-template-list-wrap li a').hasClass(templateMksEdit) && $('body').hasClass(mksEdit) && content < 1) {
                console.log('loading michaels edit template');
                obj.load_template_preset(templateMksEdit);
            } else if ($('.avia-template-list-wrap li a').hasClass(templateJetSet) && $('body').hasClass(jetSet) && content < 1) {
                console.log('loading jet set template');
                obj.load_template_preset(templateJetSet);
            } else if ($('.avia-template-list-wrap li a').hasClass(templateFashion) && $('body').hasClass(fashion) && content < 1) {
                console.log('loading fashion and style template');
                obj.load_template_preset(templateFashion);
            } else {
                console.log('nothing matched here! loading nothing.');
            }
        },
         ...
         * The new element is then added to the swedenBuilder Canvas
         */
        doTargetInsert: function(element, obj, insertMethod) {
            ...
            if (section.length) {
                obj.updateTextarea();
                avia_log("update inner 6");

                obj.updateInnerTextarea(false, section);
            }

            var row = current.parents('.avia_layout_row:eq(0)');
            if (row.length) {
                obj.updateTextarea();
                avia_log("update inner 7");
        ...
        /**
         * function that gets executed if a user clicks the scissor icon and moves an EXISTING element on the swedenBuilder Canvas to another place
         */
        /*
		targetCutItem: function(element, obj)
		{
			var el = $(element),
				text = el.parents('.avia_sortable_element, .avia_layout_column, .avia_layout_section').filter(':eq(0)'),
				also_update = el.parents('.avia_layout_column:eq(0), .avia_layout_section:eq(0)'),
				target_class = 'avia-select-target';

			if(also_update.length > 1) also_update = also_update.filter('.avia_layout_section');

			this.only_one_insert = also_update;

			if(text.is('.avia_layout_column')) { target_class = 'avia-select-target-no-col';} else
			if(text.is('.avia_layout_section')){ target_class = 'avia-top-bottom-insert'; }

			obj.targetInsertActive(text, target_class);
		},
		*/
        ...
        /**
         * function that gets executed if an element on the swedenBuilder Canvas has no popup modal but is managed via directly attached
        ...
        send_to_datastorage: function(values, element_container) {
            ...
            if (section.length) {
                avia_log("update inner 10");

                this.updateInnerTextarea(false, section);
            } else if (column.length) {
                avia_log("update inner 11");
        ...
        update_builder_html: function(element_container, values) {
            ...
            var shortcode = element_container.data('shortcodehandler'),
                ...
                class_updates = element_container.find("[data-update_class_with]").attr('class', ''),
                visual_key = "",
                visual_el = "",
                visual_template = "",
                update_html = "",
            ...
            if (!element_container.is('.avia-no-visual-updates')) {
                //update elements on the swedenBuilder Canvas like text elements to reflect those changes instantly
                visual_updates.each(function() {
                    visual_el = $(this);
                    visual_key = visual_el.data('update_with');
                    visual_template = visual_el.data('update_template');

                    if (typeof values[visual_key] === "string" || typeof values[visual_key] === "number") {
                ...
                //update element classnames on the swedenBuilder Canvas to reflect visual changes instantly
    ...
    $(document).ready(function() {
        $.avia_builder = new $.swedenBuilder();
    });
...
(function($) {
    ...
    $.swedenBuilder.shortcodes = $.swedenBuilder.shortcodes || {};

    $.swedenBuilder.shortcodes.fetchShortcodeEditorElement = function(shortcode, insert_target, obj) {
    ...
    $.swedenBuilder.shortcodes.deleteItem = function(clicked, obj) {
        var $_clicked = $(clicked),
            item = $_clicked.parents('.avia_sortable_element:eq(0)'),
            parent = false;
    ...
    $.swedenBuilder.shortcodes.deleteModalSubItem = function(clicked, e) {
    ...
    $.swedenBuilder.shortcodes.appendModalSubItem = function(clicked, e) {
        e.preventDefault();
        ...
        var $_clicked = $(clicked),
            ...
            newTemplate = $(template.html()).appendTo(parent).css({
                display: "none"
            });
    ...
    $.swedenBuilder.shortcodes.changeSize = function(clicked, obj) {
        var item = $(clicked),
            ...
            sizes = [
                ['av_full_bleed', 'Full Bleed'],
                ['av_full_margin','1/1 Margin'],
                ['av_one_full', '1/1'],
                ['av_four_fifth', '4/5'],
                ['av_three_fourth', '3/4'],
                ['av_two_third', '2/3'],
