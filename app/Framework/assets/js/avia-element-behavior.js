(function($)
{
	...
	$.AviaElementBehavior.tmpl_fetcher = function()
	{
		...
		the_body.on('change', '.avia-attach-templating select, .avia-attach-templating radio, .avia-attach-templating input[type=checkbox]', function()
		{
			...
			if($(current).attr('id') == 'aviaTBlinka') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBlinka');
			} else if ($(current).attr('id') == 'aviaTBlinkb') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBlinkb');
			} else if ($(current).attr('id') == 'aviaTBlinkc') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBlinkc');
			} else if($(current).attr('id') == 'aviaTBaviaTBlinka') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBaviaTBlinka');
			} else if ($(current).attr('id') == 'aviaTBaviaTBlinkb') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBaviaTBlinkb');
			} else if ($(current).attr('id') == 'aviaTBaviaTBlinkc') {
				template	= $("#avia-tmpl-"+new_value+'-aviaTBaviaTBlinkc');
			}
				if(!template.length)
				{
