...
{
    class avia_sc_columns extends swedenShortcodeTemplate{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['tooltip'] 	= __('Creates a single full width column', 'swedenWp' );
            ...
            function editor_element($params)
            {
                ...
                $size = array('av_full_bleed' => 'Full Bleed', 'av_full_margin' => '1/1 Margin', 'av_one_full' => '1/1', 'av_one_half' => '1/2', 'av_four_fifth' => '4/5', 'av_one_third' => '1/3', 'av_one_fourth' => '1/4', 'av_one_fifth' => '1/5', 'av_two_third' => '2/3', 'av_three_fourth' => '3/4', 'av_three_fifth' => '3/5');
                ...
                $output .= "<a class='avia-smaller avia-change-col-size' href='#smaller' title='".__('Decrease Column Size','swedenWp' )."'>-</a>";
                $output .= "<span class='avia-col-size'>".$size[$name]."</span>";
                $output .= "<a class='avia-bigger avia-change-col-size'  href='#bigger' title='".__('Increase Column Size','swedenWp' )."'>+</a>";
                $output .= "<a class='avia-delete'  href='#delete' title='".__('Delete Column','swedenWp' )."'>x</a>";
                //$output .= "<a class='avia-new-target'  href='#new-target' title='".__('Move Element','swedenWp' )."'>+</a>";
                $output .= "<a class='avia-clone'  href='#clone' title='".__('Clone Column','swedenWp' )."' >".__('Clone Column','swedenWp' )."</a></div>";
            ...
            function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
            {
                ...
                /*
                WHY IS THIS COMMENTED OUT? CAN WE NUKE THIS? PLZ?  - K 5/27/14
                ...
                */
                ...
                //$has_card = strpos($content, 'card_type')>0 ? "has-card":"";
...
if ( !class_exists( 'avia_sc_columns_full_bleed' ) )
{
    class avia_sc_columns_full_bleed extends avia_sc_columns{

            function shortcode_insert_button()
            {
                $this->config['name']       = 'Full Bleed';
                $this->config['icon']       = swedenBuilder::$path['imagesURL']."sc-full.png";
                $this->config['tab']        = __('Layout Elements', 'swedenWp' );
                $this->config['order']      = 110;
                $this->config['target']     = "avia-section-drop";
                $this->config['shortcode']  = 'av_full_bleed';
                // $this->config['shortcode_nested'] = array('av_one_full','av_one_half','av_one_third','av_two_third','av_one_fourth','av_three_fourth','av_one_fifth');
                $this->config['html_renderer']  = false;
                $this->config['tinyMCE']    = array('name' => 'Full Bleed', 'instantInsert' => "[av_full_bleed first]Add Content here[/av_full_bleed]");
                // $this->config['tinyMCE']    = array('instantInsert' => "[av_full_bleed first]Add Content here[/av_full_bleed]");
                $this->config['tooltip']    = __('Creates a single column with 100% width that bleeds fully to grid edges', 'swedenWp' );
...
if ( !class_exists( 'avia_sc_columns_full_margin' ) )
{
    class avia_sc_columns_full_margin extends avia_sc_columns{

            function shortcode_insert_button()
            {
                $this->config['name']       = '1/1 Margin';
                $this->config['icon']       = swedenBuilder::$path['imagesURL']."sc-full.png";
                $this->config['tab']        = __('Layout Elements', 'swedenWp' );
                $this->config['order']      = 100;
                $this->config['target']     = "avia-section-drop";
                $this->config['shortcode']  = 'av_full_margin';
                $this->config['html_renderer']  = false;
                $this->config['tinyMCE']    = array('name' => '1/1 Margin', 'instantInsert' => "[av_full_margin first]Add Content here[/av_full_margin]");
                $this->config['tooltip']    = __('Creates a single column with margin on the left ( best used on atricle pages)', 'swedenWp' );
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
        }
    }
...
if ( !class_exists( 'avia_sc_columns_one_half' ) )
{
    class avia_sc_columns_one_half extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-half.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                ...
                $this->config['tooltip'] 	= __('Creates a single column with 50% width', 'swedenWp' );
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
        }
    }
...
if ( !class_exists( 'avia_sc_columns_one_third' ) )
{
    class avia_sc_columns_one_third extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-third.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                $this->config['order']		= 80;
                $this->config['target']		= "avia-section-drop";
                $this->config['shortcode'] 	= 'av_one_third';
                $this->config['html_renderer'] 	= false;
                $this->config['tooltip'] 	= __('Creates a single column with 33% width', 'swedenWp' );
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
...
if ( !class_exists( 'avia_sc_columns_two_third' ) )
{
    class avia_sc_columns_two_third extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-two_third.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                $this->config['order']		= 70;
                $this->config['target']		= "avia-section-drop";
                $this->config['shortcode'] 	= 'av_two_third';
                $this->config['html_renderer'] 	= false;
                $this->config['tooltip'] 	= __('Creates a single column with 67% width', 'swedenWp' );
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
                $this->config['tinyMCE'] 	= array(
                    'name' => '2/3 + 1/3',
...
if ( !class_exists( 'avia_sc_columns_one_fourth' ) )
{
    class avia_sc_columns_one_fourth extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-fourth.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                $this->config['order']		= 60;
                $this->config['target']		= "avia-section-drop";
                $this->config['shortcode'] 	= 'av_one_fourth';
                $this->config['tooltip'] 	= __('Creates a single column with 25% width', 'swedenWp' );
                $this->config['html_renderer'] 	= false;
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
                $this->config['tinyMCE'] 	= array(
...
if ( !class_exists( 'avia_sc_columns_three_fourth' ) )
{
    class avia_sc_columns_three_fourth extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-three_fourth.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                $this->config['order']		= 50;
                $this->config['target']		= "avia-section-drop";
                $this->config['shortcode'] 	= 'av_three_fourth';
                $this->config['tooltip'] 	= __('Creates a single column with 75% width', 'swedenWp' );
                $this->config['html_renderer'] 	= false;
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
                $this->config['tinyMCE'] 	= array(
...
if ( !class_exists( 'avia_sc_columns_one_fifth' ) )
{
    class avia_sc_columns_one_fifth extends avia_sc_columns{
            ...
            function shortcode_insert_button()
            {
                ...
                $this->config['icon']		= swedenBuilder::$path['imagesURL']."sc-fifth.png";
                $this->config['tab']		= __('Layout Elements', 'swedenWp' );
                $this->config['order']		= 40;
                $this->config['target']		= "avia-section-drop";
                $this->config['shortcode'] 	= 'av_one_fifth';
                $this->config['html_renderer'] 	= false;
                $this->config['tooltip'] 	= __('Creates a single column with 20% width', 'swedenWp' );
                $this->config['drag-level'] = 2;
                $this->config['drop-level'] = 2;
                $this->config['tinyMCE'] 	= array(
...
if ( !class_exists( 'avia_sc_columns_three_fifth' ) )
{
	class avia_sc_columns_three_fifth extends avia_sc_columns{
		...
		function shortcode_insert_button()
		{
			...
			$this->config['html_renderer'] 	= false;
			$this->config['tooltip'] 	= __('Creates a single column with 60% width', 'swedenWp' );
			$this->config['drag-level'] = 2;
			$this->config['drop-level'] = 2;
			$this->config['tinyMCE'] 	= array(
					'name' => '1/5 + 3/5 + 1/5',
					'instantInsert' => "[av_one_fifth first]1/5[/av_one_fifth]\n\n\n[av_three_fifth]2/5[/av_three_fifth]\n\n\n[av_one_fifth][av_one_fifth]5/5[/av_one_fifth]"
			);
		}
	}
}
if ( !class_exists( 'avia_sc_columns_four_fifth' ) )
{
	class avia_sc_columns_four_fifth extends avia_sc_columns{
		...
		function shortcode_insert_button()
		{
			...
			$this->config['html_renderer'] 	= false;
			$this->config['tooltip'] 	= __('Creates a single column with 80% width', 'swedenWp' );
			$this->config['drag-level'] = 2;
			$this->config['drop-level'] = 2;
			$this->config['tinyMCE'] 	= array(
					'name' => '4/5 + 1/5',
					'instantInsert' => "[av_four_fifth first]1/5[/av_four_fifth]\n\n\n[av_one_fifth]5/5[/av_one_fifth]"
			);
		}
	}
}
